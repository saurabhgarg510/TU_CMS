<?php
	class Student_model extends CI_Model {
		...
		function getNumComp($data){
			...
			else $result= $this->db->query("select * from complaints where roomno like '" . $data['room'] . "%' and not(status='Complete') and category='" . $_SESSION['type'] . "' and comp_type='" . $data['level'] . "'");
		...
		public function getProfile(){
		 ...
		 function getRemark($row){
			 if(isset($row['comp_id'])) {
				 $result= $this->db->query("select * from remarks where comp_id='".$row['comp_id']."' order by time desc");
				 $data=array();
				 $data['count'] = $result->num_rows();
				 foreach($result->result_array() as $row1){
					 $det['user_type']=$row1['user_type'];
					 $det['time']=$row1['time'];
					 $det['remark']=$row1['remark'];
					 array_push($data,$det);
				 }			 
				 return $data;
			 }
